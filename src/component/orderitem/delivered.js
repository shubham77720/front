...
export default  function Delivered (params) {
  ...
  const apiKey = "https://backenddata77720.onrender.com"
   ...
   const [hasMore, setHasMore] = useState(true); // To track if there are more items to load
    ...
    let [productdata, setproductdata] = useState({refundCondition:"",billno:"", billdate:"", LRNO:"" ,shippingcharges:"" ,Trackingid:"" ,Courier:"",   Deliverybydate:""})
    ...
    let token = localStorage.getItem("token")
      try {
        const response = await axios.get(`${apiKey}/api/order/fetchallordersforadmin/Delivered/${page}`,{method: 'GET',
          headers:{
...
//console.log(productdata)
      ...
      <button className="text-[16px] bg-blue-600 rounded-full w-[50%] items-center mt-3  text-white px-2 py-[3px]" onClick={()=>{update(selectedStatus,id)}}>Update</button>
        ...
        <div className="  mt-3 w-[1300px] h-[510px]">
           <div className="bg-[#464646] flex  sticky top-3 z-20 text-white mb-2  px-2 items-center lg: ">
                <div className="  w-[140px]  px-3 py-1 ">Bill No</div>
               <div className="border-l-[1px] border-white w-[130px]  px-3 py-1 ">Order Id</div>
               <div className="border-l-[1px] border-white w-[250px]  px-3 py-1 ">Product</div>
 ...
 const bdate = new Date(person.Deliverybydate);
 const boptions = { year: 'numeric', month: '2-digit', day: '2-digit', };
let Deliverybydate = bdate.toLocaleDateString('en-IN', boptions);
Deliverybydate = Deliverybydate.split('/').join('-');
 return (
    <>
  
           <div  key={person._id} className="bg-[#f2f2f2] flex   text-black w-[1300px]  px-2 items-center ">
               <Link to="/home/Searchpage" onClick={() => setsingleorderid(person._id)} className=" w-[140px] overflow-x-auto px-3 py-1 ">{person.Billno}</Link>
               ...
               <Link to="/home/Searchpage" onClick={() => setsingleorderid(person._id)} className=" w-[250px] overflow-x-auto px-3 py-1 ">{person.Product}</Link>
               ...
               <Link to="/home/Searchpage" onClick={() => setsingleorderid(person._id)} className=" w-[100px] overflow-x-auto px-3 py-1 ">{Deliverybydate}</Link>
...
<div className="opstion  ">
         ...
         <option className={`text-[12px] cursor-pointer ${selectedStatus=="DTO"&&'bg-blue-400'}`} onClick={()=>{changestatus("DTO",person._id)}} value="pending">DTO</option>
