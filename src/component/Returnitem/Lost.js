   ...
   let [productdata, setproductdata] = useState({   billno:"", billdate:"", LRNO:"" ,shippingcharges:"",claimdate:"",claimrequired:"" ,claimapplied:"",claimamount:"",claimstatus:"",Trackingid:"" ,Courier:"",   Deliverybydate:""})
    ...
    useEffect(() => {
      ...
      let token =localStorage.getItem('token')
        try {
          const response = await axios.get(`${apiKey}/api/order/fetchallordersforadmin/Lost/${page}`,{method: 'GET',
            headers:{
      ...
      <button className="text-[16px] bg-blue-600 rounded-full w-[50%] items-center mt-3  text-white px-2 py-[3px]" onClick={()=>{update(selectedStatus,id)}}>Update</button>
         ...
         <div className="  mt-4 w-[1650px] h-[510px]">
            <div className="bg-[#464646] flex  sticky top-3 z-20 text-white mb-2  px-2 items-center ">
                ...
                <div className="border-l-[1px] border-white w-[160px]  px-3 py-1 ">Return Status</div>
                <div className="border-l-[1px] border-white w-[200px]  px-3 py-1 ">Lost Date</div>
                  
            </div>
            {data?.orders?.map((person) => {
 
 const bdate = new Date(person.Lostdate);
 const boptions = { year: 'numeric', month: '2-digit', day: '2-digit', };
let Lostdate = bdate.toLocaleDateString('en-IN', boptions);
Lostdate = Lostdate.split('/').join('-');
 return (
            <div  key={person._id} className="bg-[#f2f2f2] flex   text-black  px-2  items-center ">
                ...
                <Link to="/home/Searchpage" onClick={() => setsingleorderid(person._id)} className="w-[160px] overflow-x-auto px-3 py-1 ">{person.Retrunstatus}</Link>
                <Link to="/home/Searchpage" onClick={() => setsingleorderid(person._id)} className=" w-[200px] overflow-x-auto px-3 py-1 ">{Lostdate}</Link>
