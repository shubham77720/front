...
export default  function DTO (params) {
   ...
   const [hasMore, setHasMore] = useState(true); // To track if there are more items to load
    ...
    const Fetchproduct = async (page) => {
      ...
      try {
        ...
        const response = await axios.get(`${apiKey}/api/order/fetchallordersforadmin/DTO/${page}`, { headers });
 ...
 let changestatus =(status,iddata)=>{
  ...
  let update =async(status,id)=>{
    ...
    const removeSerialNumber = (numbeDTORemove) => {
      setSerialNumbers(prevSerialNumbers =>
        prevSerialNumbers.filter(number => number !== numbeDTORemove)
      );
    };
...
<div className="  mt-4 w-[1350px] h-[510px]">
            <div className="bg-[#464646] flex  sticky top-3 z-20 text-white mb-2  px-2 items-center ">
                ...
                <div className="border-l-[1px] border-white w-[160px]  px-3 py-1 ">DTO Date</div>
                 
            </div>
            {data?.orders?.map((person) => {
  const bdate = new Date(person.DTOdate);
  const boptions = { year: 'numeric', month: '2-digit', day: '2-digit', };
 let DTOdate = bdate.toLocaleDateString('en-IN', boptions);
 DTOdate = DTOdate.split('/').join('-');
         //console.log(person)
 return (
            <div  key={person._id} className="bg-[#f2f2f2] flex   text-black  px-2  items-center ">
                ...
                <Link to="/home/Searchpage" onClick={() => setsingleorderid(person._id)} className=" w-[160px] overflow-x-auto px-3 py-1 ">{DTOdate}</Link>
...
<div className="opstion  ">
        ...
        <option className={`text-[12px] cursor-pointer ${selectedStatus=="Received"&&'bg-blue-400'}`} onClick={()=>{changestatus("Received",person._id)}} value="pending">Received</option>
